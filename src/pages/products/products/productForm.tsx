  ...
  const onSubmit = async (formData: any) => {
    const payload = {
      ...
      branches: formData.branches.map((b: any) => ({
        ...
        is_active: b.is_active !== undefined ? b.is_active : true, // Fixing is_active for branches
      })),
      ...
      // Ensure all account values are numbers or default to 0
      sale_account: formData.sale_account ? Number(formData.sale_account) : 0,
      purchase_account: formData.purchase_account ? Number(formData.purchase_account) : 0,
      sale_return_account: formData.sale_return_account ? Number(formData.sale_return_account) : 0,
      purchase_return_account: formData.purchase_return_account ? Number(formData.purchase_return_account) : 0,
    };
    ...
    <>
      ...
      {isUpdate && isLoading ? (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <ComponentCard title="Basic Information" icon={<Package className="text-blue-500 w-5 h-5" />}>
          ...
          <ComponentCard title="Pricing & Inventory" icon={<DollarSign className="text-green-500 w-5 h-5" />}>
          ...
          <ComponentCard title="Classification" icon={<Tag className="text-red-500 w-5 h-5" />}>
            <div className="grid grid-cols-1 gap-4">
              <div>
                <Label htmlFor="category_id">Category</Label>
                <select {...register("category_id", { valueAsNumber: true })} className={selectStyles}>
              ...
              <div>
                ...
                <select {...register("sub_category_id", { valueAsNumber: true })} className={selectStyles}>
                ...
                {errors.sub_category_id && (
                  <p className="text-red-500 text-sm">{errors?.sub_category_id.message}</p>
                )}
              </div>

   <div>
                <Label htmlFor="sub_category_id">Sub Category</Label>
                <select {...register("warranty_id", { valueAsNumber: true })} className={selectStyles}>
                  <option value="">Select Sub Category</option>
                  {warranties?.map((subCategory) => (
                    <option key={subCategory.id} value={subCategory.id}>
                      {subCategory.warranty_name_en}
                    </option>
                  ))}
                </select>
                {errors.warranty_id && (
                  <p className="text-red-500 text-sm">{errors?.warranty_id.message}</p>
                )}
                </div>
                
                   <div>
                <Label htmlFor="brand_id">Sub Category</Label>
                <select {...register("brand_id", { valueAsNumber: true })} className={selectStyles}>
                  <option value="">Select Sub Category</option>
                  {brands?.map((subCategory) => (
                    <option key={subCategory.id} value={subCategory.id}>
                      {subCategory.brand_name_en}
                    </option>
                  ))}
                </select>
                {errors.brand_id && (
              ...
              <div>
                ...
                <select {...register("tax_id", { valueAsNumber: true })} className={selectStyles}>
                ...
                {errors.tax_id && (
                  <p className="text-red-500 text-sm">{errors?.tax_id.message}</p>
                )}
              </div>
            </div>
          </ComponentCard>
          ...
          {/* Units Section */}
          <ComponentCard title="Units" icon={<Layers className="text-yellow-500 w-5 h-5" />}>
            <div className="grid grid-cols-1 gap-4">
              <div>
                <Label htmlFor="unit_id">Primary Unit</Label>
                <select {...register("unit_id", { valueAsNumber: true })} className={selectStyles}>
              ...
              <div>
                ...
                <select
                  {...register("sub_units.0.id", { valueAsNumber: true, setValueAs: (v) => (v === "" ? undefined : Number(v)) })}
                ...
                {errors.sub_units?.[0]?.id && (
                  <p className="text-red-500 text-sm">{errors.sub_units[0].id.message}</p>
                )}
              </div>

                <div>
                <Label htmlFor="default_purchase_unit">Default Sale Unit</Label>
                <Input
                  type="text"
          ...
          <ComponentCard title="Branch & Status" icon={<GitBranch className="text-purple-500 w-5 h-5" />}>
            <div className="grid grid-cols-1 gap-4">
              <div>
                <Label htmlFor="branches.0.branch_id">Branch</Label>
                <select
                  {...register("branches.0.branch_id", { valueAsNumber: true })}
                  className={selectStyles}
                >
                ...
                {errors.branches?.[0]?.branch_id && (
                  <p className="text-red-500 text-sm">{errors.branches[0].branch_id.message}</p>
                )}
              </div>
              ...
              <div key="is_active">
                ...
                <Switch
                  label="is_active"
                  defaultChecked={productData?.is_active ?? false}
          ...
          <div className="flex justify-end mt-6">
            <button
              ...
              disabled={isSubmitting || addProduct.isPending || updateProduct.isPending}
            >
