...
export default function Settings() {
  return (
    <div className="p-6 bg-gray-50 min-h-screen px-6 sm:px-16 md:px-24 lg:px-32 text-sm">
      {/* Header */}
      <div className="bg-white p-4 shadow-md rounded-lg mb-6 flex justify-between items-center">
        ...
        <button className="flex items-center text-gray-600 hover:text-red-500 text-sm">
      ...
      <div className="flex justify-center mb-6">
        <div className="relative w-full max-w-md">
          ...
          <input type="text" placeholder="Search your settings" className="pl-10 border rounded-md w-full p-2 text-sm" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
        {categories.map((category, index) => (
          <div key={index} className="bg-white shadow-md hover:shadow-lg transition rounded-lg p-3 w-56 mx-auto">
            ...
            <ul className="space-y-2">
              {category.links.map((link, i) => (
                <li
                  key={i}
                  className="cursor-pointer text-gray-600 hover:text-blue-600 hover:bg-blue-100 px-2 py-1 rounded-md transition text-sm"
                >
                  {link}
                </li>
              ))}
